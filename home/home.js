...
function displayWhaleTransactions(transactions) {
    ...
    transactions.slice(0, 3).forEach((tx) => {
        ...
        txElement.className = "whale-alert-item";

        txElement.innerHTML = `
    <div class="alert-coin">
        <img src="${tx.image}" alt="${tx.coin}" class="coin-icon">
        <span>${tx.coin}</span>
    </div>
    <div class="alert-info">
        <div class="alert-amount">${numberWithCommas(
            tx.amount
        )} ($${numberWithCommas(tx.value)})</div>
        <div class="alert-path">${tx.from} → ${tx.to}</div>
        <div class="alert-time">${timeAgo}</div>
    </div>
    `;

        alertsWrapper.appendChild(txElement);
    });
...
// Function to fetch crypto news from CoinGecko
async function fetchCryptoNews() {
    try {
        
        const response = await fetch('https://min-api.cryptocompare.com/data/v2/news/?lang=EN&sortOrder=popular&limit=1');
...
function displayCryptoNews(newsItems) {
    ...
    // We're just displaying one featured news item
    const news = newsItems[0];

    const newsElement = document.createElement('div');
    newsElement.className = 'crypto-news-item';

    // Create a nice formatted published time
    ...
    <div class="news-content">
      ...
      <p class="news-excerpt">${truncateText(news.body, 120)}</p>
      <div class="news-meta">
